...
Subfinder() {
	...
	printf "                        \r"
	subfinder -nW -silent -d $domain 1> tmp-subfinder-$domain 2>/dev/null
...
Amass() {
	...
	printf "                        \r"
	# amass enum -config amass-conf.ini -d $domain 1> tmp-amass-$domain 2>/dev/null
	amass enum -d $domain 1 -active > tmp-amass-$domain 2>/dev/null
